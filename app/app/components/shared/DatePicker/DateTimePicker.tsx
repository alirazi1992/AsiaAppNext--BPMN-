...
import { DateTimePickerProps } from './Models/models'
...
const DateTimePicker: React.FC<DateTimePickerProps> = (props) => {
  ...
  const timeSelected = (time: string) => {
    ...
    setInputValue(
      moment(selectedDay + ' ' + time, 'jYYYYjMMjDD HH:mm').format(formatted)
    );
  };
        ...
        {openPicker && (
          <div className={'JDatePicker ' + customClass}>
            <div className="JDheader">
              <div className="right border-6 border-red-300">
                <Years
              ...
              <div className="left">
                <TimePicker
                  ...
                  changeEvent={(returnedTime) => timeSelected(returnedTime)}
                  selectedTime={selectedTime || moment().format('HH:mm')}
                />
              </div>
            </div>
            <Months
            ...
            />
            <div>
              <button onClick={(e) => submitHandler(e)} className={`${!themeMode ||themeMode?.stateMode ? 'lightText' : 'darkText'} text-green-500 JDsubmit`}>
                تایید
              </button>
              <button className={`${!themeMode ||themeMode?.stateMode ? 'lightText' : 'darkText'} text-red-500 JDcancel`} onClick={cancelPicker}>
                بستن
              </button>
            </div>
          </div>
